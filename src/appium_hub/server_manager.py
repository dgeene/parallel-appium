...
class AppiumServerManager:
    ...
    def __init__(self, port: int, session_id: str, log_dir: str = "logs"):
        ...
        self.log_file = os.path.join(
            self.log_dir, f"appium_server_{session_id}_{port}.log"
        )
    ...
    def start(self, timeout: int = 30) -> bool:
        ...
        with self._lock:
            if self.is_running:
                self.logger.warning(
                    f"Server for session {self.session_id} is already running"
                )
                return True

            try:
                self.logger.info(
                    f"Starting Appium server for session {self.session_id} on port {self.port}"
                )
                ...
                if self._wait_for_server_ready(timeout):
                    ...
                    self.logger.info(
                        f"Appium server started successfully on port {self.port}"
                    )
                    return True
                else:
                    self.logger.error(
                        f"Failed to start Appium server on port {self.port} within {timeout}s"
                    )
    ...
    def stop(self) -> bool:
        ...
        with self._lock:
            if not self.is_running:
                self.logger.warning(
                    f"Server for session {self.session_id} is not running"
                )
                return True

            try:
                self.logger.info(
                    f"Stopping Appium server for session {self.session_id}"
                )

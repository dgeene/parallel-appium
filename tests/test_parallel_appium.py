...
class TestParallelAppium:
    ...
    def driver_session(self, hub_url, android_capabilities):
        ...
        driver = webdriver.Remote(command_executor=f"{service_url}", options=options)
    ...
    def test_basic_interaction(self, driver_session):
        ...
        print(
            f"Interaction test completed on worker: {pytest.current_pytest_worker_id}"
        )
    ...
    def test_multiple_activities(self, driver_session):
        ...
        print(
            f"Multiple activities test completed on worker: {pytest.current_pytest_worker_id}"
        )
...
class TestHubManagement:
    ...
    def test_create_and_delete_session(self, hub_url):
        ...
        with httpx.Client() as client:
            # Create session
            response = client.post(
                f"{hub_url}/session", json={"capabilities": capabilities}, timeout=60.0
            )
            ...
            if response.status_code == 200:
                ...
                delete_response = client.delete(
                    f"{hub_url}/session/{hub_session_id}", timeout=30.0
                )
